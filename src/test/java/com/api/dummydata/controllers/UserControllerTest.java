...
class UserControllerTest {
    ...
    public void getAllUsersInFrench() {
        webTestClient.get().uri(urlBase + "?lang=fr")
                ...
                .consumeWith(users -> {
                    ...
                    assertEquals(response.get(0).getAddress().getCountry(), "France");
                });
    }
    ...
    public void getAllUsersInPortuguese() {
        webTestClient.get().uri(urlBase + "?lang=pt")
                ...
                .consumeWith(users -> {
                    List<User> response = users.getResponseBody();
                    assertNotNull(response);
                    assertEquals(response.get(0).getAddress().getCountry(), "Brasil");
                });
    }
    ...
    public void getAllUsersInEnglish() {
        webTestClient.get().uri(urlBase + "?lang=en")
                ...
                .consumeWith(users -> {
                    List<User> response = users.getResponseBody();
                    assertNotNull(response);
                    assertEquals(response.get(0).getAddress().getCountry(), "United States of America");
                });
    }
    ...
    public void getTenUsersInFrench() {
        webTestClient.get().uri(urlBase + "?size=10&lang=fr")
                ...
                .hasSize(10)
                .consumeWith(users -> {
                    assertNotNull(users.getResponseBody());
                    User firstUser = users.getResponseBody().get(0);
                    assertEquals(firstUser.getAddress().getCountry(), "France");
                });
    }
    ...
    public void getUserById() {
        webTestClient.get().uri(urlBase + "/{id}", 1)
                ...
                .consumeWith(user -> {
                    assertNotNull(user.getResponseBody());
                    assertEquals(1, user.getResponseBody().getId());
                });
    }
    ...
    public void getUserByIdInFrench() {
        webTestClient.get().uri(urlBase + "/{id}?lang=fr", 1)
                ...
                .consumeWith(user -> {
                    assertNotNull(user.getResponseBody());
                    assertEquals("Jean", user.getResponseBody().getFirstName());
                });
    }
